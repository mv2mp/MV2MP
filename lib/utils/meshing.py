...
def generate_mesh(func, verts, level_set=0, res_init=32, res_up=3, point_batch=5000) -> trimesh.Trimesh:
    ...
    connected_comp: Sequence[trimesh.Trimesh] = meshexport.split(only_watertight=False)
    assert len(connected_comp), "No connected component found"
    max_comp = max(connected_comp, key=lambda x: x.area)
    return max_comp
