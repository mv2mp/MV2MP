...
class UniformSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, take_sphere_intersection=False, far=-1):
        ...
        self.take_sphere_intersection = take_sphere_intersection  # we have true
...
class ErrorBoundSampler(RaySampler):
    def __init__(
        ...
        self.uniform_sampler = UniformSampler(
            scene_bounding_sphere, near, N_samples_eval, take_sphere_intersection=inverse_sphere_bg
        )
    ...
    def get_z_vals(self, person_number, ray_dirs, cam_loc, model, cond, smpl_tfs, eval_mode, smpl_verts):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            model.implicit_networks[person_number].eval()
            with torch.no_grad():
                samples_sdf = model.sdf_func_with_smpl_deformer(
                    person_number, points_flat, cond, smpl_tfs, smpl_verts=smpl_verts
                )[0]
            model.implicit_networks[person_number].train()
            if samples_idx is not None:
                sdf_merge = torch.cat(
                    [sdf.reshape(-1, z_vals.shape[1] - samples.shape[1]), samples_sdf.reshape(-1, samples.shape[1])], -1
                )
            ...
            if not_converge and total_iters < self.max_total_iters:
                ...
                bins = z_vals
                error_per_section = (
                    torch.exp(-d_star / beta.unsqueeze(-1)) * (dists[:, :-1] ** 2.0) / (4 * beta.unsqueeze(-1) ** 2)
                )
                error_integral = torch.cumsum(error_per_section, dim=-1)
                bound_opacity = (torch.clamp(torch.exp(error_integral), max=1.0e6) - 1.0) * transmittance[:, :-1]
